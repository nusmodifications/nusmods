...
describe('searchVenue()', () => {
  ...
  test('should find venues based on search', () => {
    ...
    // Partial match
    expect(searchVenue(venueInfo, 'L'))
      .toEqual(venues('LT1', 'lt2', 'LT17'));

    expect(searchVenue(venueInfo, 'T1'))
      .toEqual(venues('LT1', 'LT17'));

    expect(searchVenue(venueInfo, '0'))
      .toEqual(venues('S11-0302', 'CQT/SR0622'));

    // Non-existent venue
    expect(searchVenue(venueInfo, 'cofeve'))
      .toEqual({});
  });
...
describe('filterAvailability()', () => {
  test('should find venues based on search options', () => {
    expect(filterAvailability(venueInfo, {
      day: 0, // Monday
      time: 10, // 10am
      duration: 1, // 1 hour
