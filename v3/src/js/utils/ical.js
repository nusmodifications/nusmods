...
import { RawLesson, Module, Semester } from 'types/modules';
...
export function isTutorial(lesson) {
...
export function datesForAcademicWeeks(start: Date, week: number | string): Array<Date> {
...
function iCalEventForLesson(lesson: RawLesson, module: Module, semester: Semester, firstDayOfSchool: Date) {
  ...
  start.setHours(lesson.StartTime.slice(0, 2));
  // set end time
  const end = new Date(start.getTime());
  end.setHours(lesson.EndTime.slice(0, 2));
...
export function iCalForTimetable(
    ...
    moduleData: { [key: ModuleCode]: Module }, year = config.academicYear): Array<IcalEvent> {
  const firstDayOfSchool = new Date(academicCalendar[year][semester].start);
  const events = _.flatMap(timetable, (lessonConfig, moduleCode) =>
