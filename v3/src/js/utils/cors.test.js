...
test('mergeBiddingStats should merge bidding from different groups', () => {
  ...
  expect(mergedStats.find(stats =>
    stats.Round === '2A' &&
    stats.AcadYear === '2017/2018' &&
    stats.Semester === '1'))
    .toMatchObject({
...
test('findQuota should handle modules with no round 1 bidding correctly', () => {
  const ay2015s2 = mergeBiddingStats(ges1021.CorsBiddingStats).filter(stats =>
    stats.AcadYear === '2015/2016' &&
    stats.Semester === '2');
...
test('findQuota should use round 1 quota estimates if available', () => {
  const ay2017 = mergeBiddingStats(acc2002.CorsBiddingStats).filter(stats =>
    stats.Semester === '1' &&
    stats.AcadYear === '2017/2018');
