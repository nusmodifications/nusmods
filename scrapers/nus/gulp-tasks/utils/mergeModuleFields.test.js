...
describe('mergeModuleFields', () => {
  ...
  it('throws when module codes are incorrect', () => {
    expect(() =>
      testMergeModule({ ModuleCode: testModuleCode }, { ModuleCode: 'test1' }),
    ).toThrow();
    expect(() =>
      testMergeModule({ ModuleCode: 'test1' }, { ModuleCode: testModuleCode }),
    ).toThrow();
  });

  it('does not throws when module codes are the correct', () => {
    expect(() =>
      testMergeModule({ ModuleCode: testModuleCode }, { ModuleCode: testModuleCode }),
    ).not.toThrow();
  });

  it('merges whichever side that has data', () => {
    const testModule = { Data: 'testData' };
    expect(testMergeModule(testModule, { Data: '' })).toEqual(testModule);
    expect(testMergeModule({ Data: '' }, testModule)).toEqual(testModule);
  });

  it('removes null data', () => {
    const testModules = [{ Data: 'nil' }, { Data: 'n.a.' }, { Data: 'none' }, { Data: 'null' }];
    const relevantModule = { Data: 'relevant' };
    testModules.forEach((mod) => {
      expect(testMergeModule(mod, relevantModule)).toEqual(relevantModule);
      expect(testMergeModule(relevantModule, mod)).toEqual(relevantModule);
    });
  });
  ...
  it('merges whichever side that has strictly more words', () => {
    ...
    expect(testMergeModule(testModule, testModule1)).toEqual(testModule);
    expect(testMergeModule(testModule1, testModule)).toEqual(testModule);
  });
  ...
  it('logs the difference when unable to differentiate [info level]', () => {
    ...
    expect(testMergeModule(testModule, testModule1)).toEqual(testModule1);
