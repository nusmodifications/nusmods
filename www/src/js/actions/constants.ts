/** moduleBank constants **/

export const FETCH_MODULE = 'FETCH_MODULE'; // Action to fetch modules
export const FETCH_MODULE_LIST = 'FETCH_MODULE_LIST';
export const UPDATE_MODULE_TIMESTAMP = 'UPDATE_MODULE_TIMESTAMP';
export const REMOVE_LRU_MODULE = 'REMOVE_LRU_MODULE';
export const FETCH_ARCHIVE_MODULE = 'FETCH_ARCHIVE_MODULE'; // Action to fetch module from previous years

/** undoHistory constants **/

export const UNDO = 'UNDO';
export const REDO = 'REDO';

/** export constant(s) **/

export const SET_EXPORTED_DATA = 'SET_EXPORTED_DATA';
